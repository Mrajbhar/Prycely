...
import { useAllOrders } from '../../features/orders/useAdminOrders';
...
export default function AdminDashboardPage() {
  ...
  const { data: recentOrders } = useAllOrders(1);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="price rounded border border-line bg-surface px-3 py-1.5 text-xs font-medium text-ink-soft">
          Last 30 days
        </span>
      </div>
      ...
      {statsLoading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard
            ...
            hint={`Avg order ${formatPrice(stats?.averageOrderValue ?? 0)}`}
            tone="accent"
          />
          <StatCard
            label="Orders"
          ...
          <StatCard
            ...
            hint={`${stats?.lowStockProducts ?? 0} low on stock`}
            tone="ink"
          />
          <StatCard
            label="Customers"
          ...
          <table className="w-full text-sm">
            <tbody className="divide-y divide-line">
              {recentOrders?.items.slice(0, 6).map((order) => (
                <tr key={order.id}>
                  <td className="py-2.5 pr-3">
                    <Link
                      to={`/orders/${order.id}`}
                      className="price text-xs font-bold text-ink hover:text-accent"
                    >
                      {order.orderNumber}
                    </Link>
                  </td>
                  <td className="py-2.5 pr-3 text-xs text-ink-soft">{order.userEmail ?? '—'}</td>
                  ...
                  <td className="price py-2.5 text-right text-xs font-bold text-ink">
              ...
              {(!recentOrders || recentOrders.items.length === 0) && (
                <tr>
                  <td className="py-6 text-center text-xs text-muted">No orders yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <Card title="Top products">
          <ul className="divide-y divide-line">
            {topProducts?.map((p: TopProduct, i) => (
              <li key={p.productId} className="flex items-center gap-3 py-2.5">
                ...
                <div className="text-right">
                  <p className="price text-xs font-bold text-ink">{p.unitsSold} sold</p>
                  <p className="price text-[10px] text-muted">{formatPrice(p.revenue)}</p>
                </div>
              </li>
            ))}
            ...
            {lowStock?.map((p: LowStockProduct) => (
              <li key={p.productId} className="flex items-center gap-3 py-2.5">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm text-ink">{p.productName}</p>
                  <p className="price text-[10px] text-muted">SKU {p.sku}</p>
                </div>
                <span
